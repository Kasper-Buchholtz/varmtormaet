...
import { SANITY_API_VERSION } from '../shopify/constants'
...
export default (props: ShopifyDocumentActionProps): DocumentActionDescription | undefined => {
  ...
  if (type === 'product') {
    dialog = {
      message: (
        <Stack space={4}>
          <Text>Slet det aktuelle produkt og alle tilknyttede varianter i dit datasæt.</Text>
          <Text weight="medium">Intet indhold på Shopify vil blive slettet.</Text>
        </Stack>
      ),
      onCancel: onComplete,
      onConfirm: async () => {
  ...
  if (type === 'collection') {
    dialog = {
      message: (
        <Stack space={4}>
          <Text>Slet den aktuelle kollektion i dit datasæt.</Text>
          <Text weight="medium">Intet indhold på Shopify vil blive slettet.</Text>
        </Stack>
      ),
      onCancel: onComplete,
