...
function searchTree({ tree: root, searchTerm }: { tree: Tree; searchTerm: string }) {
  ...
  function searchInTree(tree: Tree) {
    for (const key in tree) {
      ...
      // Helper to normalize Danish chars
      ...
      const normalizedSlug = item.slug ? normalize(item.slug) : ''

      if (
        normalizedTitle.startsWith(nq) ||
        normalizedSlug.startsWith(nq) ||
        normalizedTitle.split(' ').some((word) => word.startsWith(nq)) ||
        normalizedSlug
          .split('/')
          .some((word) => word.startsWith(nq))
      ) {
